    ...
    <form action={action} className="min-w-sm">
      ...
      {state?.errors?.submit && (
        <div
          aria-atomic="true"
          aria-live="polite"
          className="alert alert-error mb-2 flex h-10 text-white"
          role="alert"
        >
          <svg
      ...
      {emailVerified && (
        <p className="my-2 rounded-xl bg-green-300 px-4 py-2 text-sm">
...
function Button() {
  ...
  return (
    <button
      className={`btn btn-primary btn-h-10 btn-round my-2 w-full ${pending ? "btn-disabled" : ""}`}
      disabled={pending}
      type="submit"
    >
